...
- (void)processMessage:(NSDictionary *)resp {
    ...
    [self.delegate witDidGraspIntent:intent confidence:resp[@"outcome"][@"confidence"] entities:entities body:resp[kWitKeyBody] error:nil];
    [self dispatchIntent:intent withEntities:entities andBody:resp[kWitKeyBody]];
}

- (void)error:(NSError*)e {
    [self.delegate witDidGraspIntent:nil confidence:nil entities:nil body:nil error:e];
}
...
- (void)observeNotifications {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(start)
                                                 name:kRiriStartedSystemSoundCompleted object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(audiostart:)
                                                 name:kWitNotificationAudioStart object:nil];
}
